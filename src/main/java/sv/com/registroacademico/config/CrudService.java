...
public abstract class CrudService<T extends BaseEntity<ID>, ID extends Serializable> extends BaseCrud<T, ID> {
	...
	protected <T extends BaseEntity<ID>> Collection<T> bulkSave(Collection<T> entities) {
	...
	private <T extends BaseEntity<ID>> T persistOrMerge(T t) {
		if (t.getId() == null) {
			em.persist(t);
			return t;
		} else {
